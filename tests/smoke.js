...
async function main() {
  ...
  assert(mainSource.includes('win.setOpacity(0)') && mainSource.includes("ipcMain.on('window-mode-painted'") && mainSource.includes('revealMode'), 'mode switches hide the frame across the resize and reveal it only once painted');
  assert(rendererSource.includes('notifyModePainted') && rendererSource.includes('requestAnimationFrame'), 'renderer confirms the new layout was painted before the window is revealed');
